...
土日祝の空きが新しく出たときにメールで通知します。**自治体ごとに確認が
終わり次第、その自治体分だけ個別にメールを送ります**(全自治体の確認完了を
待たず、件名にも自治体名が入ります)。
...
| 葛飾区 | 公共施設予約システム | 館ごとに異なる (奥戸4枠 / 水元5枠) | 2枠以上連続、奥戸の大体育室全面・エイトホール・水元のメインアリーナ全面 |
| 松戸市 | 公共施設インターネット予約システム | 6枠 (2時間単位 9-11〜19-21) | 2枠以上連続、小金原・柿ノ木台・常盤平・松戸運動公園の体育館「全面」のみ |
| 足立区 | P-kashikan | 4枠 (9-12/12:30-15/15:30-18/18:30-21) | 2枠以上連続、バレーボール2面以上使える6館の「全面」のみ |
...
足立区は青梅市と同じP-kashikanシステムのため、共通エンジン
`scrapers/p_kashikan.py`(青梅市もこれを使うようリファクタ済み)を使い、
`scrapers/adachi.py` にはサブドメイン・目的コード・時間枠の違いだけを
定義しています。データセルには実際の利用枠(9-12等)の間に「隙間セル」が
挟まる構造のため、`real_cell_indices` で実枠の位置だけを抽出しています
(隙間セルを含めると「2枠連続」の判定が本来の意図からずれてしまうため)。

葛飾区も独自系統(JSP)で `scrapers/katsushika.py` に実装しています。時間枠が館ごとに
異なるため画面から読み取り、カレンダーが「予約あり」の日は時間帯を開いても空きが
ないことを利用して、開く日を絞って高速化しています。対象の館・部屋は同ファイルの
`TARGET_BUILDINGS` で指定します。水元総スポ体育館は利用目的が「バレーボール」だと
メインアリーナがＡ面／Ｂ面でしか出てこないため、この館だけ「その他」で検索して
全面を取得しています。

松戸市も別系統(比較的新しいUI)で `scrapers/matsudo.py` に実装しています。週表示の
表が他自治体と逆で「行=時間帯・列=日付」になっている点と、検索フォームが折りたたみ
式で、1度検索した後にトップへ戻ると前回開いたままの状態で表示される(その状態を
見ずに毎回トグルを押すと逆に閉じてしまう)点に対応しています。対象の館・部屋は
同ファイルの `TARGET_ROOMS` で指定します。
...
- どちらのシステムも単純なHTTPリクエストでは空き状況を取得できないため、
  ...
  しています。
- サーバー負荷軽減のため、実行頻度は控えめ(既定30分おき)に設定しています。
...
4. 以降は `.github/workflows/check.yml` の cron 設定に従い、JST 6:00〜23:00の
   ...
   - Playwright(ブラウザ自動化)を使い、自治体ごとに順番にアクセスするため、
     対応自治体が増えるほど1回の実行時間が伸びます(2026年9月時点、7自治体で
     11〜17分程度)。**このリポジトリを非公開(private)のまま使う場合は
     GitHub Actionsの無料枠(月2000分)にすぐ達してしまうので注意してください**
     (例: 30分おき×1回15分として概算 月換算で1万分超)。公開(public)リポジトリ
     ならActionsの無料枠は無制限です。頻度を下げたい場合は
     `.github/workflows/check.yml` の cron 設定を編集してください。
...
- `facility_excludes`: 施設名にこの文字列を含む施設を対象から除外する
  (`facility_filters` と併用可。例: 青梅市の天ヶ瀬体育館は分割面だけ除外)
